...
public class RetryClientFilterTest
{
	...
	{
		RestClientCreator.createCached().resource("http://localhost:2222/TheResource/test").get(String.class);
	...
	{
		long start = System.currentTimeMillis();
		ClientResponse response = RestClientCreator.createCached().resource("http://localhost:2222/TheResource/test").post(ClientResponse.class, "stuff");
	...
	{
		long start = System.currentTimeMillis();
		RestClientCreator.createCached().resource("http://localhost:2222/TheResource/test").get(List.class);
		long end = System.currentTimeMillis();
		assertTrue(RESPONSE_TIME > (end - start));
	}
	...
	public void testUnknownHost() throws Exception
	{
		try
		{
			RestClientCreator.createCached().resource("http://blah:2222/TheResource/test").get(String.class);
		}
	...
	public void testInvalidPort() throws Exception
	{
		...
		try
		{
			RestClientCreator.createCached().resource("http://localhost:2223/TheResource/test").get(String.class);
		}
		catch (ClientHandlerException e)
		{
