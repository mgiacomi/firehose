...
public class BatchTest
{
	...
	public void testToJsonAndBack() throws Exception
	{
		...
		assertEquals(batch.getLastEventTime(), tb1.getLastEventTime());
		assertEquals(batch.getBytes(), tb1.getBytes());
		assertEquals(batch.getEvents().size(), tb1.getEvents().size());

		for (int i = 0; i < batch.getEvents().size(); i++)
		{
			Message e1 = batch.getEvents().get(i);
	...
	public void testSpeedToJsonAndBack100k() throws Exception
	{
		...
		for (int i = 0; i < 100000; i++)
		{
			...
			bigBatch.addEvent(new Message("1", "2", testString.getBytes()));
		}
