...
public class EventResource
{
	...
	@Inject
	public EventResource(BucketMetaDataCache bucketMetaDataCache, EventService eventService)
	{
	...
	{
		eventService.addEvent(customer, bucket, payload);
			...
			StreamingOutput out = new StreamingOutput()
			{
				public void write(OutputStream outputStream) throws IOException, WebApplicationException
				{
					...
					try
					{
						...
						if (sec > -1)
						{
							recordsWritten = eventService.writeEventsToOutputStream(bucketMetaData, new DateTime(year, month, day, hour, min, sec), outputStream, recordsWritten);
						}
						...
						else if (min > -1)
						{
							for (int i = 0; i < 60; i = i + periodSeconds)
							{
								...
								{
									recordsWritten = eventService.writeEventsToOutputStream(bucketMetaData, dateTime, outputStream, recordsWritten);
								}
							}
						}
